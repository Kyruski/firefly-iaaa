...
def test_validate_grant_type(validator: OauthRequestValidators, oauth_request_list: List[Request]):
    ...
    for i in range(6):
        for x in range(4):
            ...
            assert validator.validate_grant_type('', grant_types[x], oauth_request_list[i].client, oauth_request_list[i]) == (((i % 4) == x) or ((i in (0, 2, 3, 4)) and (x == 1)))

            # Same as above, but with (x + 1) instead of (x) to check incorrect types (probably not needed)
            assert validator.validate_grant_type('', grant_types[(x + 1) % 4], oauth_request_list[i].client, oauth_request_list[i]) == (((i % 4) == ((x + 1) % 4)) or ((i in (0, 2, 3, 4)) and (x + 1 == 1)))
