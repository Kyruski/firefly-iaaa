...
async def test_introspect_token_endpoint(client, kernel, registry, bearer_messages: List[ff.Message]):
    ...
    first_response = await client.post('/firefly-iaaa/iaaa/introspect', data=json.dumps(data), headers={'Referer': 'abc'})
    await assert_error(first_response)

    data['token'] = bearer_messages[0]['active'].refresh_token
    second_response = await client.post('/firefly-iaaa/iaaa/introspect', data=json.dumps(data), headers={'Referer': 'abc'})
    ...
    third_response = await client.post('/firefly-iaaa/iaaa/introspect', data=json.dumps(data), headers={'Referer': 'abc'})
    ...
    fourth_response = await client.post('/firefly-iaaa/iaaa/introspect', data=json.dumps(data), headers={'Referer': 'abc'})
