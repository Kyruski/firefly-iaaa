...
async def test_create_token_code_from_auth_endpoint(client, registry, bearer_messages: List[ff.Message], kernel):
    ...
    first_response = await client.post('/firefly-iaaa/iaaa/create-token', data=json.dumps(data), headers={'Referer': 'abc'})
    resp = json.loads(await first_response.text())
    assert 'error' in resp

    data['grant_type'] = bearer_messages[0]['active'].grant_type
    second_response = await client.post('/firefly-iaaa/iaaa/create-token', data=json.dumps(data), headers={'Referer': 'abc'})
    resp = json.loads(await second_response.text())
    assert 'error' in resp

    data['client_id'] = bearer_messages[0]['active'].client_id
    third_response = await client.post('/firefly-iaaa/iaaa/create-token', data=json.dumps(data), headers={'Referer': 'abc'})
    resp = json.loads(await third_response.text())
    assert 'error' in resp

    data['password'] = bearer_messages[0]['active'].password
    fourth_response = await client.post('/firefly-iaaa/iaaa/create-token', data=json.dumps(data), headers={'Referer': 'abc'})
    resp = json.loads(await fourth_response.text())
    assert 'error' in resp

    data['code'] = bearer_messages[0]['active'].code
    fifth_response = await client.post('/firefly-iaaa/iaaa/create-token', data=json.dumps(data), headers={'Referer': 'abc'})
    resp = json.loads(await fifth_response.text())
    assert 'error' in resp

    data['code_verifier'] = bearer_messages[0]['active'].code_verifier
    final_response = await client.post('/firefly-iaaa/iaaa/create-token', data=json.dumps(data), headers={'Referer': 'abc'})
